...
/****************************************************************/
  ...
  export default {
    ...
    methods:{
      // 售卖信息选择
      changeIsChecked(spuSaleAttrValue,arr){
        arr.forEach((item)=>{
          item.isChecked = '0';
        });
      ...
      async addshopcar(){
          // dispatch只能带两个参数?
          // this.$store.dispatch('addOrUpdateShopCart',this.$route.params.skuId,this.skuNum)
          ...
          try{
            ...
            this.spuSaleAttrList.forEach((item)=>{
              if(item.saleAttrName == '颜色'){
                item.spuSaleAttrValueList.forEach((item)=>{
                  if(item.isChecked == '1') color = item.saleAttrValueName;
                })
              }else{
                item.spuSaleAttrValueList.forEach((item)=>{
                  if(item.isChecked == '1') version = item.saleAttrValueName;
                })
              }
            });
            ...
            sessionStorage.setItem('COLORANDVERSION',JSON.stringify({color,version}));
